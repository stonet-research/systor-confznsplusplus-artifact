...
if __name__ == "__main__":
    ...
    for key, value in data.items():
        if "bw" in key:
            continue
        numjobs = int(key.split('iod_')[1].split('-')[0])
        print(numjobs)
        y = 0
    ...
    ax.plot(write50_32_iops, write50_32, markersize = 4, marker = '*',   label=" 50% finish-64KiB")
    ax.plot(write50_16_iops, write50_16, markersize = 4, marker = '*',   label=" 50% finish-32KiB")
    ax.plot(write50_8_iops, write50_8, markersize = 4, marker = '*',   label=" 50% finish-16KiB")
